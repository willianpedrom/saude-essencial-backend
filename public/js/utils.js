...
export function openClientOffcanvas(client) {
        ...
        <div class="offcanvas-header">
          <div style="display:flex;align-items:center;gap:12px">
            ...
            <div>
              ...
              <div id="oc-guardian-name" style="font-size:0.85rem;color:var(--text-muted);display:none;margin-top:2px"></div>
   ...
   tipoRadios.forEach(radio => {
     radio.addEventListener('change', async (e) => {
       try {
         ...
         const { store } = await import('./store.js?v=1003');
  ...
  if (dossiesContainer) {
    import('./store.js?v=1003').then(async ({ store }) => {
      try {
        ...
        anamneses.forEach(a => {
          if (a.dados?.guardian_name || a.dados?.personal?.guardian_name) {
             foundGuardian = a.dados.guardian_name || a.dados.personal.guardian_name;
          }
          ...
          btnOpen.onclick = async () => {
             ...
             const freshAnamneses = await import('./store.js?v=1003').then(m => m.store.getClientAnamneses(client.id)).catch(() => []);
             const freshA = freshAnamneses.find(x => x.id === a.id) || a;
             const { auth } = await import('./store.js?v=1003');
             ...
             if (navigator.clipboard && window.ClipboardItem) {
                 try {
                     const fetchHash = async () => {
                         const { api } = await import('./store.js?v=1003');
        ...
        if (foundGuardian) {
            const gEl = document.getElementById('oc-guardian-name');
            if (gEl) {
                gEl.innerHTML = `👤 Responsável: <strong>${foundGuardian}</strong>`;
