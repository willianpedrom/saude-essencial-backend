...
export function openClientOffcanvas(client) {
   ...
   tipoRadios.forEach(radio => {
     radio.addEventListener('change', async (e) => {
       try {
         ...
         const { store } = await import('./store.js?v=1008');
  ...
  if (dossiesContainer) {
    import('./store.js?v=1008').then(async ({ store }) => {
      try {
        ...
        anamneses.forEach(a => {
          ...
          btnOpen.onclick = async () => {
             ...
             const freshAnamneses = await import('./store.js?v=1008').then(m => m.store.getClientAnamneses(client.id)).catch(() => []);
             const freshA = freshAnamneses.find(x => x.id === a.id) || a;
             const { auth } = await import('./store.js?v=1008');
             ...
             if (navigator.clipboard && window.ClipboardItem) {
                 try {
                     const fetchHash = async () => {
                         const { api } = await import('./store.js?v=1008');
