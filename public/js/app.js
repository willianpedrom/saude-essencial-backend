...
import { auth } from './store.js?v=1003';
import { Router, setupGlobalShortcuts } from './utils.js?v=1003';
...
document.addEventListener('open-anamnese', async (e) => {
    ...
    if (client) {
        const { showAnamneseModal } = await import('./pages/Clients.js?v=1003');
...
const router = new Router({
    '/': async (p) => {
        ...
        const { renderLogin } = await import('./pages/Login.js?v=1003');
        renderLogin(router);
    },
    '/dashboard': guard(async (params) => {
        const { renderDashboard } = await import('./pages/Dashboard.js?v=1003');
        renderDashboard(router, params);
    }),
    '/clients': guard(async (params) => {
        const { renderClients } = await import('./pages/Clients.js?v=1003');
        renderClients(router, params);
    }),
    '/links': guard(async (params) => {
        const { renderLinks } = await import('./pages/Links.js?v=1003');
        renderLinks(router, params);
    }),
    '/anamnesis': guard(async (params) => {
        const { renderAnamnesisList } = await import('./pages/Anamneses.js?v=1003');
        renderAnamnesisList(router, params);
    }),
    '/schedule': guard(async () => {
        const { renderSchedule } = await import('./pages/Schedule.js?v=1003');
    ...
    '/testimonials': guard(async () => {
        const { renderTestimonials } = await import('./pages/Extras.js?v=1003');
        renderTestimonials(router);
    }),
    '/purchases': guard(async () => {
        const { renderPurchases } = await import('./pages/Extras.js?v=1003');
        renderPurchases(router);
    }),
    '/assinatura': guard(async () => {
        const { renderAssinatura } = await import('./pages/Assinatura.js?v=1003');
        renderAssinatura(router);
    }),
    '/profile': guard(async () => {
        const { renderProfile } = await import('./pages/ProfileV31.js?v=1003');
        renderProfile(router);
    }),
    '/admin': guard(async () => {
        const { renderAdmin } = await import('./pages/Admin.js?v=1003');
        renderAdmin(router);
    }),
    '/pipeline': guard(async () => {
        const { renderPipeline } = await import('./pages/Pipeline.js?v=1003');
        renderPipeline(router);
    }),
    '/insights': guard(async () => {
        const { renderInsights } = await import('./pages/Insights.js?v=1003');
        renderInsights(router);
    }),
    '/integrations': guard(async () => {
        const { renderIntegrations } = await import('./pages/Integracoes.js?v=1003');
        renderIntegrations(router);
    }),
    '/prospecting': guard(async () => {
        const { renderProspecting } = await import('./pages/Prospecting.js?v=1003');
        renderProspecting(router);
    }),
    '/estoque': guard(async () => {
        const { renderInventory } = await import('./pages/Inventory.js?v=1003');
        renderInventory(router);
    }),

    // Public routes (no auth required)
    '/anamnese/:token': async ({ token }) => {
        const { renderPublicAnamnesis } = await import('./pages/PublicAnamnesis.js?v=1003');
        renderPublicAnamnesis(router, token);
    },
    '/vendas/capture/:token': async ({ token }) => {
        const { renderSalesAnamnesis } = await import('./pages/SalesAnamnesis.js?v=1003');
        renderSalesAnamnesis(router, token);
    },
    '/protocolo': async (params) => {
        const { renderReport } = await import('./pages/Report.js?v=1003');
    ...
    '/business-report': async (params) => {
        const { renderBusinessReport } = await import('./pages/BusinessReport.js?v=1003');
        renderBusinessReport(router, params?.data);
    },
    '/recomendacao-uau': async (params) => {
        const { renderRecomendacaoUau } = await import('./pages/RecomendacaoUau.js?v=1003');
        renderRecomendacaoUau(router, params?.data);
    },
    '/depoimento/:slug': async ({ slug }) => {
        const { renderPublicTestimonial } = await import('./pages/PublicTestimonial.js?v=1003');
        renderPublicTestimonial(router, slug);
    },
    '/p/:slug': async ({ slug }) => {
        const { renderPublicProfile } = await import('./pages/PublicProfile.js?v=1003');
        renderPublicProfile(router, slug);
    },
    '/vendas': async () => {
        const { renderLandingPage } = await import('./pages/LandingPage.js?v=1003');
    ...
    '/reset-password': async () => {
        const { renderResetPassword } = await import('./pages/Login.js?v=1003');
