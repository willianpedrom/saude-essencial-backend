...
const router = new Router({
    '/': async (p) => {
        ...
        const { renderLogin } = await import('./pages/Login.js?v=1028');
        renderLogin(router);
    },
    '/login': async (params) => {
        if (auth.isLoggedIn) return router.navigate('/dashboard');
    ...
    '/admin': guard(async () => {
        const { renderAdmin } = await import('./pages/Admin.js?v=1028');
        renderAdmin(router);
    }),
    ...
    '/reset-password': async () => {
        const { renderResetPassword } = await import('./pages/Login.js?v=1028');
