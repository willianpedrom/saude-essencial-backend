import { analyzeAnamnesis } from '../analysis.js?v=1005';
import { PROTOCOLS } from '../protocols.js?v=1005';
import { OILS_DATABASE, LIVING_KIT } from '../oils.js?v=1005';
import { getConsultantTitle } from '../utils.js?v=1005';
     ...
     try {
       ...
       const data = await res.json();
        ...
        if (data.tipo === 'recrutamento' || data.subtipo === 'recrutamento') {
          const { renderBusinessReport } = await import('./BusinessReport.js?v=1005');
  ...
  <div class="report-page">
    <div class="report-card" style="max-width:750px">
      ...
      <div class="report-body" style="padding:24px 28px">
        ...
        <!-- 3. ROTINA DIÁRIA (PARTE PÚBLICA / MANHÃ) -->
        ...
        <!-- ÁREA RESTRITA: Tarde + Noite + Específicos + Resultados Clínicos -->
        <div style="position:relative;margin-bottom:24px">
          ...
          <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);text-align:center;width:100%;z-index:10px">
            <div style="background:rgba(255,255,255,0.95);padding:14px 24px;border-radius:20px;display:inline-block;box-shadow:0 10px 25px rgba(0,0,0,0.15);border:1px solid #ddd;max-width:90%">
              ...
              <div style="font-size:0.8rem;color:#666;line-height:1.4">Chame sua consultora para liberar a chave do prognóstico<br>e ver sua rotina completa.</div>
            </div>
          </div>
          ` : ''}
        </div>
          ...
          <div style="background:linear-gradient(135deg,#f0fff4,#fffbeb);border:1px solid #d4e8c2;border-radius:12px;padding:24px;margin:24px 0;text-align:center">
            ...
            <a class="report-cta" href="https://wa.me/${waPhone}?text=${whatsappMsg}" target="_blank" style="display:flex;align-items:center;justify-content:center;gap:12px;padding:16px 20px;background:linear-gradient(135deg,#25d366,#128c7e);color:white;border-radius:12px;text-decoration:none;margin-bottom:16px;box-shadow:0 8px 20px rgba(37,211,102,0.3);transition:transform 0.2s">
              ...
              <div style="text-align:left">
                <strong style="display:block;font-size:0.95rem">ENVIE UMA MENSAGEM NO WHATSAPP PARA DESBLOQUEAR SEU PROTOCOLO</strong>
      ...
      <div style="background:linear-gradient(135deg, #1e293b, #0f172a);padding:28px 24px;border-top:2px solid #334155;text-align:center;">
        ...
        <h3 style="color:#fcd34d;font-size:1.1rem;margin-bottom:12px">AJUDE UM AMIGO A MELHORAR A SAÚDE DE FORMA NATURAL</h3>
        <p style="color:#cbd5e1;font-size:0.9rem;max-width:480px;margin:0 auto 20px;line-height:1.5">
          Gostou deste protocolo exclusivo? Compartilhe o link abaixo com seus amigos e familiares que precisam fazer uma avaliação exclusiva e gratuita.
        </p>
