 ...
 * Retorna uma lista de todas as equipes do sistema com estatísticas agregadas de engajamento
 */
router.get('/equipes', async (req, res) => {
    try {
        ...
        console.error('[AdminGetEquipes] Error:', err.message);
        res.status(500).json({ error: 'Erro ao listar equipes administrativas.' });
    }
});
...
router.get('/debug-equipes', async (req, res) => {
    try {
        ...
        const rawConsultoras = await pool.query("SELECT id, nome, email, equipe_id, role FROM consultoras WHERE nome ILIKE '%Carla%' OR email ILIKE '%carla%' OR role = 'admin' OR equipe_id IS NOT NULL");
