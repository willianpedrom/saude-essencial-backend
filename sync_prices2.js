...
updatedOils = updatedOils.replace(/'([^']+)': \{([^}]+)sizes:\s*\[(.*?)\]([^}]*)\}/g, (match, oilName, before, arrayInner, after) => {
    ...
    for (let sObj of sizesArr) {
        ...
        if (!p && oilName.toLowerCase() === 'zengest' && sObj.size.includes('10')) p = getDotPrices('zengest touch', '10ml touch');
        if (!p && oilName.toLowerCase() === 'clarycalm' && sObj.size.includes('10')) p = getDotPrices('clarycalm', '10ml touch');
        if (!p && oilName.toLowerCase() === 'pasttense' && sObj.size.includes('10')) p = getDotPrices('pasttense', '10ml touch');
        if (!p && oilName.toLowerCase() === 'intune' && sObj.size.includes('10')) p = getDotPrices('intune', '10ml touch');
