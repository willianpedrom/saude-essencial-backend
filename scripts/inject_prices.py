...
data_file = "public/js/data.js"
with open(data_file, 'r', encoding='utf-8') as f:
    content = f.read()

for key, sizes in prices.items():
    sizes_json = json.dumps(sizes, ensure_ascii=False)
    # Procurar pela chave, e colocar o campo sizes logo em seguida antes de fechar a chave (})
    # 'Lavanda': { nameEn: 'Lavender', cat: 'single', ... }
    # regex to match from key to the closing brace that does NOT contain another key
    pattern = r"('" + key + r"'\s*:\s*\{[^}]*)(\})"
    content = re.sub(pattern, r"\g<1>, sizes: " + sizes_json + r" \g<2>", content)

with open(data_file, 'w', encoding='utf-8') as f:
    f.write(content)

print("Preços injetados via script Python")
